...
{
public:
  ClassKenLM(const std::string &line, const std::string &file, FactorType factorType, bool lazy, const std::string& classPath);

protected:
  std::map<std::string, std::string> m_word_to_class;

  lm::WordIndex TranslateID(const Word &word) const {
      std::map<std::string, std::string>::const_iterator it = m_word_to_class.find(SPrint(word.GetString(this->m_factorType)));
