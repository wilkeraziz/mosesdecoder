...
void LoadWordToClassMapping(const std::string& path, std::map<std::string, std::string>& mapping)
{
    mapping.clear();
...
{
    LoadWordToClassMapping(classPath, m_word_to_class);
}
        ...
        const std::string& classPath)
{
  ...
  if (lm::ngram::RecognizeBinary(file.c_str(), model_type)) {
    ...
    case lm::ngram::PROBING:
      return new ClassKenLM<lm::ngram::ProbingModel>(line, file, factorType, lazy, classPath);
    case lm::ngram::REST_PROBING:
      return new ClassKenLM<lm::ngram::RestProbingModel>(line, file, factorType, lazy, classPath);
    case lm::ngram::TRIE:
      return new ClassKenLM<lm::ngram::TrieModel>(line, file, factorType, lazy, classPath);
    case lm::ngram::QUANT_TRIE:
      return new ClassKenLM<lm::ngram::QuantTrieModel>(line, file, factorType, lazy, classPath);
    case lm::ngram::ARRAY_TRIE:
      return new ClassKenLM<lm::ngram::ArrayTrieModel>(line, file, factorType, lazy, classPath);
    case lm::ngram::QUANT_ARRAY_TRIE:
      return new ClassKenLM<lm::ngram::QuantArrayTrieModel>(line, file, factorType, lazy, classPath);
    default:
...
LanguageModel* ConstructClassKenLM(const std::string& line)
{
  ...
  return ConstructClassKenLM(line, filePath, factorType, lazy, classPath);
}
